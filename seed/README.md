...
```
1. Oracle BANK   → BANK_ACCOUNT (20건)
2. Oracle STOCK  → SECURITIES_ACCOUNT (20건) + STOCK_HOLDING (46건)
3. Oracle TRANS  → USER_MASTER (10건) + USER_ACCOUNT_MAPPING (40건)
4. PostgreSQL    → ALTER 보완 → USERS → USER_PROFILE → PIN_AUTH
                → VIRTUAL_SALARY_SETTING → LINKED_FINANCIAL_ACCOUNT
```

> PostgreSQL과 Oracle은 순서에 의존성이 없으므로 동시에 실행해도 무방합니다.

---

## OpenStack 서버 (docker-compose-core + docker-compose-app)
...
```
=== [TEST] PostgreSQL Seed (postgres/finance) ===
  [PG] 00_alter_schema.sql...
  [PG] 01_seed_users.sql...
  [PG] 02_seed_user_profile.sql...
  [PG] 03_seed_pin_auth.sql...
  [PG] 04_seed_virtual_salary.sql...
  [PG] 05_seed_linked_accounts.sql...
=== [TEST] Oracle Seed ===
  [Oracle] 01_seed_bank.sql as BANK...
  [Oracle] 02_seed_stock.sql as STOCK...
  [Oracle] 03_seed_trans.sql as TRANS...
=== [TEST] Seed Done ===
```
...
```bash
# 사용자 확인
docker exec -i postgres psql -U admin -d finance -c "SELECT user_id, user_name, email FROM users WHERE user_id BETWEEN 22 AND 31;"

# 은행 계좌 확인
docker exec -i oracle sqlplus -S BANK/bank123@XEPDB1 <<'EOF'
SELECT ACCOUNT_ID, USER_ID, BANK_CODE, BALANCE FROM BANK_ACCOUNT WHERE ACCOUNT_ID BETWEEN 2001 AND 2020;
EXIT;
EOF

# 증권 계좌 확인
docker exec -i oracle sqlplus -S STOCK/stock123@XEPDB1 <<'EOF'
SELECT SECURITIES_ACCOUNT_ID, USER_ID, BROKER_CODE, CASH_BALANCE FROM SECURITIES_ACCOUNT WHERE SECURITIES_ACCOUNT_ID BETWEEN 1001 AND 1020;
EXIT;
EOF
```
...
## 로컬 환경 (docker-compose-onpremise)

**대상 컨테이너**

| DB         | 컨테이너                   | DB명                   |
|------------|--------------------------|----------------------|
| PostgreSQL | `postgres-operational`   | `finance_operational` |
| Oracle     | `oracle-onpremise`       | `XEPDB1`              |
...
```
=== [LOCAL] PostgreSQL Seed (postgres-operational) ===
  [PG] 00_alter_schema.sql...
  [PG] 01_seed_users.sql...
  ...
=== [LOCAL] Oracle Seed ===
...
> 잔액이 큰 계좌(1001, 1005, 1008, 1011, 1016, 1019)는 3종목, 나머지는 2종목.

---

## 문제 해결

### Oracle 컨테이너가 아직 준비되지 않은 경우

```bash
# Healthy 상태 확인 후 실행
docker ps | grep oracle
# STATUS: healthy 확인 후 스크립트 실행
```

### psql 실행 시 `$` 기호 오류

`-c` 옵션으로 BCrypt 해시를 직접 넣으면 쉘이 `$`를 변수로 인식합니다.  
반드시 `-f -` + 파일 리다이렉트 방식을 사용하세요 (스크립트가 이미 이 방식으로 동작합니다).

```bash
# 올바른 방법 (스크립트 내부 동작 방식)
